...
def get_all_settings():
    ...
    if not query.exec("SELECT settings_key, value, default_value FROM tbl_settings"):
        logging.debug(f"Error: Could not load settings (get_all_settings())-> {query.lastError().text()}")
        return settings_map
...
def get_setting_by_key(settings_key, default_value=None):
    ...
    if not query.exec():
        logging.debug(f"Error: Failed to execute query(get_setting_by_key())-> {query.lastError().text()}")
    ...
    if query.next():
        ...
        logging.debug(f"settings_key not found: {settings_key}. Returning default value.")
...
def get_settings_by_keys(keys):
    ...
    # if not db.isOpen():
    #     logging.debug("Error: Database is not open.")
    #     return settings_map
    ...
    if not query.exec():
        logging.debug(f"Error: Failed to execute query(get_settings_by_keys()) -> {query.lastError().text()}")
        return settings_map
...
def update_setting(settings_key, value):
    ...
    if not query.exec():
        logging.debug(f"Error: Could not update setting(update_setting()) -> {query.lastError().text()}")
...
def update_settings(settings_dict):
    ...
    if not db.isValid() or not db.isOpen():
        logging.debug("Error: Database connection is not valid or open(update_settings())")
        return False

    if not db.transaction():
        logging.debug(f"Error: Could not start transaction(update_settings() -> {db.lastError().text()}")
        return False
    ...
    for settings_key, value in settings_dict.items():
        ...
        if not query.exec():
            logging.debug(f"Error: Could not update setting(update_settings()) -> {query.lastError().text()}")
            db.rollback()
            return False

    if not db.commit():
        logging.debug(f"Error: Could not commit transaction(update_settings()) -> {query.lastError().text()}")
        db.rollback()
        return False
